...
function selectHeroMedia(indexOrThumb, eventOrIndex) {
  // Supports two call shapes:
  //   - CSP delegation: selectHeroMedia(paramString, event)
  //   - Legacy:         selectHeroMedia(thumbElement, index)
  let index, thumb;
  if (typeof indexOrThumb === 'string' || typeof indexOrThumb === 'number') {
    index = parseInt(indexOrThumb, 10);
    const ev = eventOrIndex;
    thumb = (ev && ev.target && ev.target.closest)
      ? ev.target.closest('.hero-thumb')
      : heroThumbs[index];
  } else {
    thumb = indexOrThumb;
    index = eventOrIndex;
