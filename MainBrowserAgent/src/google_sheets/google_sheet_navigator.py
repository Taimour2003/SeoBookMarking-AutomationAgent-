...
def save_session_data(signup_data: dict, parsed_entries: list):
    ...
    try:
        payload = {"signup_data": signup_data, "parsed_entries": parsed_entries}
...
def load_session_data() -> tuple[dict, list]:
    ...
    return (
        {},
        [],
    )  # Return empty dict and list if no session data is found or an error occurs
...
async def load_sheet_navigator(force_reload: bool = False):
    ...
    print("Current Data from Sheet: ", current_data)

    signup_data = {
        key: current_data.get(key, "") for key in target_keys if current_data.get(key)
    }
    ...
    first_name = signup_data.get("first_name", "").strip()
    print("First Name: ", first_name)
    last_name = signup_data.get("last_name", "").strip()
